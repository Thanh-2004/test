...
def PointPath(node, path, interval, df, AllData, fuel, distance, level):   
    ...
    while priorityQueue.empty() == False:
        ...
        print("Node đang khám phá:", current[0][0])
        print("Khoảng cách cần đi:", current[0][1])
        traverse_level = -current[1]
        print(traverse_level)
        ...
        if current[0][1] > fuel: ## Fuel constraint is violated
            ...
            ## If there exist a station satisfy
                ...
                print("Node đang khám phá:", current[0])
                print("Khoảng cách cần đi:", current[1])
                distance += current[1]
                store_distance.append(current[1])
                fuel = fuel_capacity
                path.append(current[0])
                print("Cập nhật",path ,distance)
                print("Xăng còn:", fuel)
                print("--------------------------")
                # if level != 1:
...
def StationPath(node, path, interval, df, dfp , AllData, fuel, distance, level):  ## Hàng dọc là điểm bắt đầu, Hàng ngang là điểm cần đến
    ...
    for i in range (interval_norm(interval[level-1])):
        ...
        if successor == "Finish":
            ...
            try:
                for j in range (interval_norm(interval[level])):
                    pQueue.put((AllData["Distances to finish point"].iloc[j+interval[level][0]] + df[node].iloc[i + interval[level-1][0]] + dfp[level][successor].iloc[j] + distance,
                                [df[node].index[i + interval[level-1][0]], df[node].iloc[i + interval[level-1][0]]]))
...
for i in range (len(Intervals)-1):
    a = dfSt[Intervals[i][0]: Intervals[i][1]].min().min()
    lst.append(a)
...
distance = 0
path = ["Start"]
# fuel_capacity = int(math.ceil(max(lst))) # range (6x - 2xx)
fuel_capacity = 100
fuel = fuel_capacity
